...
export function ChallengesManager() {
  ...
  const handleReorder = async (c: Challenge, direction: 'up' | 'down') => {
    ...
    if (!swapWith) return
    await Promise.all([
      fetch(`/api/admin/challenges/${c.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sort_order: swapWith.sort_order }),
      }),
      fetch(`/api/admin/challenges/${swapWith.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
          ...
          <div className="rounded-xl border border-border/50 bg-muted/20 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>Title</Label>
                <Input
                  placeholder="e.g. Hit 2,000x on Gates of Olympus"
                  value={form.title}
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  id="challenge-active"
                />
                <Label htmlFor="challenge-active" className="cursor-pointer">Active (visible on site)</Label>
              </div>
