...
import { Trophy, MessageCircle, ExternalLink, Zap } from 'lucide-react'

export const metadata: Metadata = {
  title: 'Challenges | AceBet — R2K2',
  description:
    'Complete AceBet challenges and win prizes. Hit specific multipliers, land big wins, and claim your reward through Discord.',
}
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <nav className="flex flex-wrap gap-2" aria-label="AceBet perks">
            {PERKS_NAV.map((item) => {
              const active = item.href.includes('challenges')
              return (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          {challenges.length === 0 ? (
            <div className="rounded-2xl border border-border/40 bg-card/30 py-20 text-center">
              ...
              <p className="text-lg font-semibold text-muted-foreground">No active challenges right now</p>
              <p className="text-sm text-muted-foreground/60 mt-1">Check back soon — new challenges drop regularly.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {challenges.map((challenge) => (
                <div
                  key={challenge.id}
                  className="group relative rounded-2xl border border-border/40 bg-gradient-to-b from-card/70 to-card/30 overflow-hidden transition-all hover:border-amber-500/30 hover:shadow-[0_0_40px_-16px_rgba(245,158,11,0.3)]"
                >
                  {/* Image */}
                  <div className="relative w-full aspect-video bg-muted/30 overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/90 backdrop-blur-sm px-3 py-1 text-[11px] font-black uppercase tracking-wider text-emerald-950 shadow-lg">
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-1 gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black text-emerald-400">{challenge.prize}</span>
                      </div>
                      <Link
                        href={DISCORD_URL}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 rounded-full bg-gradient-to-b from-blue-400 to-blue-600 px-4 py-2 text-[11px] font-black uppercase tracking-wider text-white shadow-[0_0_16px_-6px_rgba(96,165,250,0.7)] transition-all hover:from-blue-300 hover:to-blue-500 hover:shadow-[0_0_20px_-4px_rgba(96,165,250,0.9)] active:scale-95 whitespace-nowrap"
                      >
          ...
          <div className="rounded-xl border border-border/40 bg-card/30 px-4 py-4 space-y-1.5">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-mono font-bold text-foreground">R2K2</span>. Open a ticket in
              the Discord server and post your screen recording or screenshot as proof. Prizes are
              paid once verified — one claim per challenge per user.
            </p>
          </div>
