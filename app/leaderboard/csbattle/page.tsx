...
// Config
// ---------------------------------------------------------------------------
// Competition runs for 30 days starting on launch day. Query start is pulled
// back 1 day to capture wagers near the timezone boundary; the displayed range
// stays on the true start date.
const START_DATE = '2026-07-25'      // query start (1 day early for boundary)
...
export default function CsBattleLeaderboard() {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `/api/csbattle/affiliates?startDate=${START_DATE}&endDate=${END_DATE}`,
          { cache: 'no-store' }
        )
        ...
        // Sort by wager desc so ranking is always correct regardless of API order
        const list = normalizeEntries(json).sort((a, b) => getEntryWagered(b) - getEntryWagered(a))
        setEntries(list)
      } catch (err) {
  ...
  const getAvatarUrl = (avatar: string | null): string => {
    if (!avatar) return '/assets/csbattle.png'
    if (avatar.startsWith('http://') || avatar.startsWith('https://')) {
      return avatar.split('#')[0]
    }
    return '/assets/csbattle.png'
  }
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center space-y-6">
            ...
            <div className="flex justify-center">
              <a
                href="https://csbattle.com/r/r2k2"
                target="_blank"
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-border/50 overflow-hidden">
                  <div className="px-5 py-4 bg-muted/40 border-b border-border/50">
                    <h3 className="font-bold text-sm uppercase tracking-widest text-muted-foreground">How Your Score Works</h3>
                  </div>
                  <div className="divide-y divide-border/30">
                    <div className="flex items-center justify-between px-5 py-4 bg-card/50 hover:bg-muted/10 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-sm">Every Wager Counts</p>
                          <p className="text-xs text-muted-foreground">Your total wagered amount under code R2K2 is your leaderboard score</p>
                        </div>
                      </div>
                          ...
                          <div key={getEntryId(entry)} className="flex items-center justify-between rounded-xl bg-primary/5 border border-primary/20 px-4 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="relative w-8 h-8 rounded-full overflow-hidden border border-primary/30">
                                <img src={getAvatarUrl(getEntryAvatar(entry))} alt={getEntryName(entry)} className="absolute inset-0 w-full h-full object-cover" crossOrigin="anonymous" onError={(e) => { (e.target as HTMLImageElement).src = '/assets/csbattle.png' }} />
                              </div>
