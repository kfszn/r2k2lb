...
export async function POST(req: Request) {
  try {
    ...
    if (!settings?.is_open) {
      return NextResponse.json({ success: false, message: '❌ Guess The Multi is currently closed.' }, { status: 403 })
    }
    ...
    const { error } = await supabaseAdmin
      .from('predictions')
      .upsert({ type: 'gtm', kick_username: kickUsername, guess: guessNum, updated_at: new Date().toISOString() }, { onConflict: 'type,kick_username' })
    ...
    console.error('[bot/predictions/gtm] error:', err)
